...
async function openSlimBot() {
    slimbot.on("message", async message => {
        if (message.text == "/start") {
            slimbot.sendMessage(message.chat.id, helloMessage);
            ...
            if (foodList["soup"] != "") {
                var foodListString = await foodListService.foodListString(
                    foodList
                );
            ...
            slimbot.sendMessage(
                message.chat.id,
                "https://github.com/safderun/ibuYemekBotu"
            );
        }
    });
}
